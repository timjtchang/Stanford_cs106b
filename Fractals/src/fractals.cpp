...
void drawTreeHelper( GWindow& gw, double x, double y, double size, int order, int theta){
    ...
    if( order == 2 ) gw.setColor( "#2e8b75");

    theta = (theta-45+360)%360;
    for( int i=0 ; i<7 ; i++ ){

        gw.drawPolarLine( x, y, size, theta);
        drawTreeHelper( gw, x+size*cos( theta*M_PI/180), y-size*sin( theta*M_PI/180), size/2, order-1, theta);
        theta = (theta+15)%360;
...
void drawTree(GWindow& gw, double x, double y, double size, int order) {
    ...
    gw.setColor( "#8b7765" );
    gw.drawPolarLine( x+size/2, y+size/2, size/2, 270);
    drawTreeHelper( gw, x+size/2, y+size/2, size/4, order, 90);
